...
const NAV_ITEMS = [
    { label: 'Insert Inventory',          to: 'insert',            icon: 'ti-plus'        },
    { label: 'Get Inventory',             to: 'getById',           icon: 'ti-search'      },
    { label: 'Get All Inventory',         to: 'getAll',            icon: 'ti-list'        },
    { label: 'Replenish Stock',           to: 'replenish',         icon: 'ti-refresh'     },
    { label: 'Get By Product',            to: 'getByProduct',      icon: 'ti-box'         },
    { label: 'Low Stock Alerts',          to: 'getLowStock',       icon: 'ti-alert-triangle' },
    { label: 'All Inventory Paginated',   to: 'getallpaginated',   icon: 'ti-layout-rows' },
];

export default function InventoryHome() {
    const navigate = useNavigate();
    const location = useLocation();
    const isHome = location.pathname.replace(/\/$/, '') === '/Inventory';
                    ...
                    <>
                        ...
                        <div style={{
                            ...
                            <p style={{ fontSize: 13, color: 'rgba(255,255,255,.8)', position: 'relative', zIndex: 1 }}>
                                Manage your inventory — insert, update, replenish and monitor stock levels with ease
                            </p>
                        </div>
