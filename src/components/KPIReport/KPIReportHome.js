...
const NAV_ITEMS = [
    { label: 'Generate Report',  to: 'savereport',     icon: 'ti-chart-bar'   },
    ...
    { label: 'Latest By Scope',  to: 'getLatest',      icon: 'ti-clock'       },
    { label: 'By Date Range',    to: 'GetByDateRange', icon: 'ti-calendar'    },
    { label: 'Trend Analysis',   to: 'getTrend',       icon: 'ti-trending-up' },
...
const METRIC_CARDS = [
    ...
    },
    {
        label: 'Sales Growth',
        value: '≥ 0%',
        sub: 'No declining revenue',
        hint: 'Period over period growth rate',
        grad: GRAD.green,
        icon: 'ti-trending-up',
    },
    {
        ...
        value: '≤ 5%',
        sub: 'Max stock loss allowed',
        hint: 'Lost or damaged inventory ratio',
        grad: GRAD.red,
        icon: 'ti-package',
    },
];
                    ...
                    {NAV_ITEMS.map(item => (
                        <Link key={item.to} to={item.to}
                            style={{
                                ...
                                color: '#c4cfe8', textDecoration: 'none',
                                background: 'rgba(255,255,255,.05)',
                                border: '1px solid rgba(255,255,255,.08)',
                                transition: 'all .15s',
                            }}
                            onMouseEnter={e => { e.currentTarget.style.background = 'rgba(255,255,255,.12)'; e.currentTarget.style.color = '#fff'; }}
                            onMouseLeave={e => { e.currentTarget.style.background = 'rgba(255,255,255,.05)'; e.currentTarget.style.color = '#c4cfe8'; }}
                        >
                    ...
                    <>
                        ...
                        <div style={{
                            ...
                            <p style={{ fontSize: 13, color: 'rgba(255,255,255,.8)', position: 'relative', zIndex: 1 }}>
                                Compute Stock Turnover, Sales Growth and Shrinkage from live sale and inventory data
                            </p>
                            <div style={{ display: 'flex', gap: 10, marginTop: 16, position: 'relative', zIndex: 1, flexWrap: 'wrap' }}>
                                {[['3', 'KPI Metrics'], ['Auto', 'Computed'], ['Live', 'Data Source'], ['Feeds', 'Compliance']].map(([n, l]) => (
                                    <div key={l} style={{
                                        background: 'rgba(255,255,255,.16)', backdropFilter: 'blur(8px)',
                                        border: '1px solid rgba(255,255,255,.25)', borderRadius: 11,
